  ...
  async function handlePaste() {
    ...
    if (isUrl(text)) {
      ...
      if (apiKey && meta.content) {
        try {
          const ai = await api.aiExtract({ type: "link", content: meta.content, title: meta.title, apiKey });
          tags = ai.tags || [];
          summary = ai.summary || "";
        } catch {}
      }
      ...
      if (apiKey) {
        try {
          const ai = await api.aiExtract({ type: "note", content: text, apiKey });
      ...
      {!preview ? (
        <>
          ...
          <div className="flex gap-2">
            <button onClick={handlePaste} disabled={loading || !input.trim()} className="flex-1 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium disabled:opacity-50">
              {loading ? "处理中..." : "预览"}
            </button>
        ...
        <>
          {preview.cover_image && (
            <img src={preview.cover_image} alt="" className="w-full h-40 object-cover rounded-lg" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
          )}
          ...
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="添加备注...（可选）" className="w-full p-2 border border-gray-200 dark:border-gray-600 rounded-lg resize-none text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 bg-gray-50 dark:bg-gray-900 dark:text-white" rows={2} />
          <div>
            <div className="flex flex-wrap gap-1.5 mb-2">
              {tagOptions.map((tag) => (
                <button key={tag} onClick={() => toggleTag(tag)} className={`text-xs px-2 py-1 rounded-full border transition-colors ${selectedTags.includes(tag) ? "bg-blue-100 border-blue-300 text-blue-700 dark:bg-blue-900 dark:border-blue-700 dark:text-blue-300" : "border-gray-300 text-gray-600 dark:border-gray-600 dark:text-gray-400"}`}>
                  {tag}
                </button>
              ))}
            </div>
            <div className="flex gap-1">
              <input value={customTag} onChange={(e) => setCustomTag(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addCustomTag()} placeholder="自定义标签..." className="flex-1 text-xs p-1.5 border border-gray-200 dark:border-gray-600 rounded focus:outline-none focus:ring-1 focus:ring-blue-400 bg-gray-50 dark:bg-gray-900 dark:text-white" />
